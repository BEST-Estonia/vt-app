...
export default function TreasureHuntScreen() {
  ...
  const openScanFor = (company: Company) => {
    if (scanned.includes(company.id)) {
      Alert.alert(
        t("treasure.alreadyScannedTitle"),
        t("treasure.alreadyScannedBody"),
      );
      return;
    }
  ...
  const handleSubmitWinner = async () => {
    if (!fullName.trim()) {
      Alert.alert(
        t("treasure.error.enterNameTitle"),
        t("treasure.error.enterNameBody"),
      );
      return;
    }
  ...
  let handleMainButtonPress = () => {
    Alert.alert(
      t("treasure.notFinishedTitle"),
      t("treasure.notFinishedBody", { total }),
    );
  };
  ...
  return (
    ...
    >
      ...
      >
        <View className="mt-4 p-6 mb-2 items-center">
          ...
          <View className="flex-row items-center mt-3 bg-blue-50 px-4 py-2 rounded-full self-center">
            <Text className="text-base font-semibold text-blue-800 flex-shrink">
              {t("treasure.progress", { scanned: scannedCount, total })}
            </Text>
            {isFinished && (
              <View className="ml-2">
          ...
          <TouchableOpacity
            ...
            className={`mt-4 ${buttonBg} w-full max-w-xs px-6 py-3 rounded-xl shadow-md flex-row items-center justify-center`}
          >
          ...
          <View className="flex-1 items-center justify-center">
            {!permission.granted ? (
              ...
              <CameraView
                ...
                facing="back"
                onBarcodeScanned={
                  scannedRecent ? undefined : handleBarCodeScanned
                }
              />
            )}
          ...
          <View className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-xl">
            <View className="items-center mb-5">
              ...
              <Text className="text-gray-600 text-center mt-2 px-2">
                {t("treasure.winner.body")}
              </Text>
            </View>
            <Text className="text-sm font-bold text-gray-700 mb-1.5 ml-1">
              {t("treasure.winner.nameLabel")}
            </Text>
            <TextInput
              value={fullName}
              onChangeText={setFullName}
              placeholder={t("treasure.winner.namePlaceholder")}
            ...
            >
              {isSubmitting ? (
                <Text className="text-white font-semibold">
                  {t("treasure.winner.submitLoading")}
                </Text>
              ) : (
                <>
                  <Text className="text-white font-bold text-base mr-2">
                    {t("treasure.winner.submitCta")}
                  </Text>
